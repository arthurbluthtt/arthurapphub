...
- La verificación H5 separa el smoke de acceso desplegado —`/zzz` redirige a login sin cookie— del E2E autenticado de mutaciones, que requiere una sesión de prueba autorizada. El build y el TypeScript de ZZZ se validan sin mezclar el error baseline de otra app. Tras publicar `508e1aa`, el Worker confirma `401 {error:"unauthorized"}` con `Content-Type: application/json`; el contrato queda operativo en producción.
