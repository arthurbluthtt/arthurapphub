...
- La verificación H5 separa el smoke de acceso desplegado —`/zzz` redirige a login sin cookie— del E2E autenticado de mutaciones, que requiere una sesión de prueba autorizada. El build y el TypeScript de ZZZ se validan sin mezclar el error baseline de otra app. La fuente ya define errores JSON, pero el Worker publicado todavía entrega `401 text/plain`; se debe desplegar y repetir el smoke antes de dar ese contrato por operativo en producción.
