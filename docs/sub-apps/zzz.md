...
- Smoke desplegado sin sesión: `/zzz` redirige a `/?next=/zzz` y `GET /zzz/api/search?...` devuelve `401`, `Content-Type: application/json` y `{error:"unauthorized"}` sin exponer datos. El contrato quedó confirmado en producción tras publicar `508e1aa`. El E2E autenticado confirmó búsqueda/picker, alta, duplicado, búsqueda sin coincidencias y persistencia; el reorden por drag fue validado manualmente en Edge y los datos temporales se eliminaron al cierre.
